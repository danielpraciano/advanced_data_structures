...
int32_t hash_int(const ExtensibleHashTable *const eht, const void *const key) {
...
void free_int(ExtensibleHashTable *eht) {
    //??? fazer key e value depois
}

int main(int argc, char *argv[]) {
    int n = 5, size = 2;
    const char *const hash_type = "Extensible Hash";
    void *key, *value;

    printf("Enter the param n: ");
    scanf("%d", &n);

    printf("Enter the size of buckets: ");
    scanf("%d", &size);

    printf("Creating %s...\n", hash_type);

    ExtensibleHashTable *hash = eht_create(size, hash_int, compare_int, free_int);
